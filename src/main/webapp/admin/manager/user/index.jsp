...
<%@ page import="com.lxy.webbms.models.User" %>
<%@ page import="java.util.ArrayList" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ page contentType="text/html;charset=UTF-8" %>
<jsp:useBean id="mu" class="com.lxy.webbms.models.ManagerUsers" />
...
<%
    ...
    if (admin.getId() == 0)
        ...
        admin.initAdminByID();
    // 通过GET请求参数得到 userList
    String searchType = request.getParameter("searchType");
    String searchData = request.getParameter("data");
    System.out.println(searchType + ":" + searchData);
    List<User> userList = new ArrayList<>(); // mu.getUsers();
    if (searchData != null) {
        switch (searchType) {
            case "name":
                userList.add(mu.getUser(mu.NAME, searchData));
                System.out.println("name breach");
                break;
            case "account":
                userList.add(mu.getUser(mu.ACCOUNT, searchData));
                break;
            case "sex":
                if (searchData.equals("男") || searchData.equals("女"))
                    userList = mu.getUsers(searchData);
                break;
            default:
                userList = mu.getUsers();
                break;
        }
    } else  userList = mu.getUsers();
%>
<!DOCTYPE html>
<html>
<head>
    ...
    <div class="navbar">
        <div class="navbar-container">
            <a href="#">
                ...
                <span class="nav-info">用户管理首页</span>
            </a>
        ...
        <div class="main-container">
            <div class="row">
                        ...
                        <table class="table" width="60%">
                            ...
                            <c:forEach items="<%= userList %>" var="user">
                                <tr>
                                    ...
                                    <td><a href="<c:url value="/admin/manager/user/update.jsp?id=${user.getId()}"/>">修改</a></td>
                                    <td><a href="<c:url value="/admin/api/deleteUser?uid=${user.getId()}&aid=${admin.getId()}"/>">删除</a></td>
                                </tr>
                                <% i ++; %>
                            </c:forEach>
                            </tbody>
                ...
                <div class="col item-3">
                    <div class="card">
                        <a href="./add.jsp?id=${admin.getId()}">添加用户</a>
                    </div>
                    <div class="card">
...
<script type="text/javascript">
    ...
    function searchBook(aid) {
        ...
        let byAccount = document.getElementById('by-account').checked
        let bySex = document.getElementById('by-sex').checked
        ...
        if (info) {
            let searchType = byName ? 'name' : (byAccount ? 'account': (bySex ? 'sex': 'none'))
