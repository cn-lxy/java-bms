...
<jsp:useBean id="user" class="com.lxy.webbms.models.User" />
<jsp:setProperty name="user" property="id" />
<%
    // 判断登录
    if (user.getId() != 0)
        user.initUserByID();
    else response.sendRedirect("/user/login.jsp");
    // 链接生成
    String href = String.format("/user/borrowDetail.jsp?id=%d&detail=", user.getId());  // => 借阅详情链接
    String a_home = String.format("/user/home.jsp?id=%s", user.getId()); // => home 链接
    // 分页
    ...
    } else bookList = book.getBooksDefault((pages - 1)*10, 10);
    // 获取书籍分类
...
<head>
    ...
    <link rel="icon" href="${pageContext.request.contextPath}/static/img/icon/home.png">
    <style>
        .mypaeg {
            display: flex;
    ...
    <div class="navbar">
        <div class="navbar-container">
            <a href="<%=a_home%>">
                ...
                <span class="nav-info">个人主页</span>
            </a>
            <span>欢迎您!&nbsp;&nbsp;<a title="点击更改个人信息" href="<c:url value="/user/profile.jsp?id="/><%=user.getId()%>"><%=user.getName()%></a></span>
        </div>
    </div>
</header>
<div>
        ...
        <div class="main-container">
            <div class="row">
                ...
                <div class="col item-1">
                    <div class="card">
                        <a href="<%=href%>1" >
                            所有借阅<span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
                            <%=user.allBorrow().size()%><span class="visually-hidden">unread messages</span>
                            </span>
                        </a>
                    </div>
                    <div class="card">
                        <a href="<%=href%>2" >
                            待还借阅<span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
                            <%=user.borrowing().size()%><span class="visually-hidden">unread messages</span>
                            </span>
                        </a>
                    </div>
                    <div class="card">
                        <a href="<%=href%>3" >
                            历史借阅<span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
                            <%=user.historyBorrow().size()%><span class="visually-hidden">unread messages</span>
                        </span>
                        </a>
                    </div>
                </div>
                        ...
                        <table class="table" width="60%">
                            ...
                            <%
                                ...
                                for (Object obj : bookList) {
                                    HashMap<String, String> m = (HashMap<String, String>) obj;
                                    i++;
                            %>
                            <tr>
                                <th scope="row"><%= i %></th>
                                ...
                                <td><%= m.get("stock") %></td>
                                <td><a href="#" onclick="borrowBook(<%= m.get("isbn") %>, <%=user.getId()%>)">借阅</a></td>
                            </tr>
                            <%
                                }
                            %>
                ...
                <div class="col item-3">
                    <div class="card">
                        ...
                        <div class="input-group input-group-sm">
                            ...
                            <button class="btn btn-outline-secondary" type="button" id="search-btn" onclick="searchBook(<%=user.getId()%>)">Search</button>
                        </div>
                                    ...
                                    for (int k = 0; k < 3; k++) {
                                       if (n < size) {
                                           type = (Map<Object, Object>) bookTypesList.get(n);
                                            n++;
                                            url = String.format("/user/home.jsp?id=%d&typeid=%s", user.getId(), type.get("id"));
                                %>
...
<script type="text/javascript">
    /*借书GET请求*/
    function borrowBook(isbn, uid) {
        let days = prompt('请输入借阅天数: ')
        if (days*1 < 0 || days*1 >60) {
            alert('借阅天数在0~60天')
            location.reload()
            return
        }
        if (days === null) {
            ...
            $.ajax({
                url: '/user/api/borrow',
                type: 'GET',
                data: { isbn: isbn, uid: uid, days: days },
                dataType: "text",
