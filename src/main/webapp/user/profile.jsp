...
<head>
    ...
    <link rel="icon" href="${pageContext.request.contextPath}/static/img/icon/profile.png">
</head>
<%
    user.initUserByID();
    ...
    <div class="navbar">
        <div class="navbar-container">
            <a href="<%=a_home%>">
                ...
                <span class="nav-info">个人信息</span>
            </a>
            <span>欢迎您!&nbsp;&nbsp;<span style="color: #7d8bd0"><%=user.getName()%></span></span>
        </div>
    </div>
</header>
<div>
...
<script>
    ...
    $(document).ready(function () {
        // 填充历史信息
        $('#name')[0].value = oldData.name
        $('#account')[0].value = oldData.account
        $('#password')[0].value = oldData.password
        $('#college')[0].value = oldData.college
        $('#birthday')[0].value = oldData.birthday
        if (oldData.sex === '男') $('#man')[0].checked = true
        else $('#women')[0].checked = true
    ...
    // 验证输入是否合法
    // TODO: 编写表单检查规则: 1.字段不能超过限定长度; 2. 所有字段未更改，提示 3. 更改密码要求重新登录
    function send() {
        let newData = {
            name: $("#name")[0].value,
        ...
        // 为 `true` 数据未发生变化
        if (oldData.check(newData)) {
            alert("数据未更改")
        } else if (checkVoid(newData)) {
            $.ajax({
                url: `/user/api/modify?id=<%=user.getId()%>`,
                type: 'POST',
                data: newData,
                success: function (data) {
                    ...
                    if (newData.password !== oldData.password) {
                        ...
                        confirm(data)
                        setTimeout(() => {
                            location.href = `/user/home.jsp?id=<%=user.getId()%>`
                        }, 500)
                    }
                },
                error: function (data) {
