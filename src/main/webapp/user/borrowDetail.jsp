...
<%
    ...
    // home 链接
    String a_home = String.format("/user/home.jsp?id=%s", user.getId());
    // 获取home页面传来的参数: detail => 1: 显示所有借阅 2: 显示未完成借阅 3: 显示历史借阅
...
<head>
    ...
    <link rel="icon" href="${pageContext.request.contextPath}/static/img/icon/library.png">
    <style>
        .row {
...
<header>
    <div class="navbar">
        <div class="navbar-container">
            ...
            <span>欢迎您!&nbsp;&nbsp;<a href="<c:url value="/user/api/profile?id="/><%=user.getId()%>"><%=user.getName()%></a></span>
        </div>
    </div>
</header>
<div>
                ...
                <div class="col item-2">
                    <div class="card">
                        <table class="table" width="60%">
                            ...
                            <tr>
                                <th scope="row"><%= i %></th>
                                <td><%= m.get("book_name") %></td>
                                <td><%= m.get("days") %></td>
                                <td><%= m.get("borrow_date") %></td>
                                <td><%= m.get("back_date")==null ? "未还" : m.get("back_date") %></td>
                                <td><a href="#" class="back" onclick="backBook(<%=user.getId()%>, <%=m.get("isbn")%>)"><%=m.get("back_date")==null ? "归还" : ""%></a></td>
                            </tr>
                            <%
                                }
                            %>
